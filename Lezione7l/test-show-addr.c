...
void my_fun()
{
	int stack_var;			/* on stack */
	static int static_var;		/* on BSS */
...
int main()
{
	int int_stack;			/* on stack */
	int * p_var_heap;		/* on stack */
	...
	fgets((char *)&p_var_heap, sizeof(p_var_heap), stdin); /* Just any memory */
	...
	printf("  v_BSS (mod):  %s\n", v_BSS);
	printf("  p_BSS:        %s\n", p_BSS);
#if 0 /* Uncomment to attempt to write in read-only */
	p_BSS[0] = 'X';
	printf("  p_BSS (mod):  %s\n", p_BSS);
#endif	
